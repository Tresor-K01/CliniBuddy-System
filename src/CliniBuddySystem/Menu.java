...
public class Menu{
   ...
   //método privado para a aba de diagnóstico
    ...
    private void gerenciarAgendamentos() {
        ...
        while(flag){
            ...
            int subMenu = lerInteiroComLimite("Sua opção: ", 0, 5); // <-- MUDANÇA (limite 5)
            switch (subMenu){
                case 0:
                    flag = false;
                    break;
  ...
  private void realizarCadastro(){
        ...
        while(flag){
            ...
            switch (temp){
                ...
                case 2:
                    ...
                    novoPaciente = this.gerenciadorPaciente.cadastrarCachorro(nome, raca, idade, peso, porte);
                    flag = false;
                    break;
                }
            }        
    ...
    private Paciente selecionarPaciente(){
        if (this.clinica.getListaDePacientes().isEmpty()) {
            System.out.println("\n-> Nenhum paciente encontrado. Cadastre um primeiro.\n");
            return null;
        }

        System.out.println("\nSelecione um paciente:");
        for (int i = 0; i < this.clinica.getListaDePacientes().size(); i++) {
            System.out.println("-> " + (i + 1) + ". " + this.clinica.getListaDePacientes().get(i));
        }

        int id = lerInteiroComLimite("Digite o número do paciente: ", 1, this.clinica.getListaDePacientes().size());
        return this.clinica.getListaDePacientes().get(id - 1);
        }
    ...
    private void exibirAgendamentos(){
        if (this.clinica.getListaDeAgendamentos().isEmpty()) {
            System.out.println("\n-> Nenhum agendamento encontrado.\n");
            return;
        }

        System.out.println("\n--- Agendamentos Registrados ---");
        for (int i = 0; i < this.clinica.getListaDeAgendamentos().size(); i++) {
            // Usa o toString() do Agendamento
            System.out.println("-> " + (i + 1) + ". " + this.clinica.getListaDeAgendamentos().get(i));
        }

        System.out.println("---------------------------------");
        System.out.print("Digite o número do agendamento para ver detalhes (ou 0 para voltar): ");
        int id = lerInteiroComLimite("", 0, this.clinica.getListaDeAgendamentos().size());

        if (id > 0) {
            Agendamento selecionado = this.clinica.getListaDeAgendamentos().get(id - 1);
    ...
    private void atualizarStatusAgendamento() {
        ...
        System.out.print(" -> Digite o novo status (Ex: Confirmado, Realizado): ");
    ...
    /**
     * Método auxiliar para selecionar um agendamento da lista.
     * (Similar ao selecionarPaciente)
     */
...
private void excluirHistorico() {
        ...
        // 1. Selecionar o histórico específico
        System.out.println("Selecione o histórico a ser excluído:");
        ...
        Historico registroParaExcluir = historicos.get(id - 1);

        // 2. Confirmar a exclusão
        System.out.println("ATENÇÃO: Isso excluirá o registro: " + registroParaExcluir);
        System.out.print("Tem certeza que deseja continuar? (S/N): ");
        String confirmacao = entrada.nextLine();

        if (confirmacao.equalsIgnoreCase("S")) {
            // 3. Chamar o controlador
            this.gerenciadorRegistros.excluirHistorico(paciente, registroParaExcluir);
    ...
    private void excluirDiagnostico() {
        ...
        // 1. Selecionar o diagnóstico específico
        System.out.println("Selecione o diagnóstico a ser excluído:");
        ...
        Diagnostico registroParaExcluir = diagnosticos.get(id - 1);
        ...
        if (confirmacao.equalsIgnoreCase("S")) {
            // 3. Chamar o controlador
            this.gerenciadorRegistros.excluirDiagnostico(paciente, registroParaExcluir);
